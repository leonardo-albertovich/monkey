...
static void mk_http1_header_cb_finished(struct mk_stream_input *in)
{
...
static void cb_http1_stream_iov_extended_free(struct mk_stream_input *in)
{
    struct mk_iov *iov;
...
int mk_http1_header_prepare(struct mk_http1_session *cs, 
                            ...
                            struct mk_server *server)
{
    ...
    /* Connection */
    if (sh->connection == 0) {
        if (cs->base.close_now == MK_FALSE) {
            if (sr->base.connection.len > 0) {
                if (sr->base.protocol != MK_HTTP_PROTOCOL_11) {
                    mk_iov_add(iov,
    ...
    if (sh->cgi == SH_NOCGI || sh->breakline == MK_HEADER_BREAKLINE) {
        if (!sr->response._extra_rows) {
            ...
            mk_iov_add(sr->response._extra_rows, mk_iov_crlf.data,
    ...
    sr->in_headers.cb_finished = mk_http1_header_cb_finished;
    ...
    if (sr->response._extra_rows) {
        ...
        in->dynamic     = MK_FALSE;
        in->cb_consumed = NULL;
        in->cb_finished = cb_http1_stream_iov_extended_free;
        in->stream      = &sr->base.stream;
        in->buffer      = sr->response._extra_rows;
        in->bytes_total = sr->response._extra_rows->total_len;

        mk_list_add_after(&sr->in_headers_extra._head,
                          &sr->in_headers._head,
                          &sr->base.stream.inputs);
    }
...
void mk_http1_header_set_http_status(struct mk_http1_request *sr, int status)
{
    mk_bug(!sr);
    sr->response.status = status;

    MK_TRACE("Set HTTP status = %i", status);
}

void mk_http1_header_response_reset(struct mk_http1_response *header)
{
    struct mk_iov *iov;

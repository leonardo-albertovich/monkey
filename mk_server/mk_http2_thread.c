...
struct mk_http2_libco_params libco_param;
...
static inline void thread_cb_init_vars()
{
    ...
    struct mk_http2_session *session = libco_param.session;
    struct mk_http2_request *request = libco_param.request;
    struct mk_thread *th = libco_param.th;
    struct mk_http2_thread *mth;
    ...
    if (type == MK_HTTP_THREAD_LIB) {
         ...
         */
        ret = mk_http2_request_end(session, session->server);
        if (ret == -1) {
            close = MK_TRUE;
        }
        else {
        ...
        mk_http2_thread_purge(mth, close);
...
struct mk_http2_thread *mk_http2_thread_create(int type,
                                               struct mk_vhost_handler *handler,
                                               struct mk_http2_session *session,
                                               struct mk_http2_request *request,
                                               int n_params,
                                               struct mk_list *params)
{
    ...
    th = mk_thread_new(sizeof(struct mk_http2_thread), NULL);
    if (!th) {
        return NULL;
    }

    mth = (struct mk_http2_thread *) MK_THREAD_DATA(th);
    if (!mth) {
        return NULL;
    }
 ...
 */
int mk_http2_thread_purge(struct mk_http2_thread *mth, int close)
{
    struct mk_sched_worker *sched;
...
int mk_http2_thread_destroy(struct mk_http2_thread *mth)
{
    struct mk_thread *th;
    ...
    th = mth->parent;
    mth->session->connection->channel.event->type = MK_EVENT_CONNECTION;
    mk_thread_destroy(th);

    return 0;
}

int mk_http2_thread_event(struct mk_event *event)
{
 ...
 * data flush requests.
 */
int mk_http2_thread_start(struct mk_http2_thread *mth)
{
    mk_http2_thread_resume(mth);
    return 0;
}
